...
const (
	QueryGetUserByEmail     = `SELECT user_id, email, nickname, "password_hash", avatar_id FROM public.user WHERE email=$1`
	QueryGetUserByNickname  = `SELECT user_id, email, nickname, "password_hash", avatar_id FROM public.user WHERE nickname=$1`
	QueryGetUserById        = `SELECT user_id, email, nickname, "password_hash", avatar_id FROM public.user WHERE user_id=$1`
	QueryRegisterUser       = `INSERT INTO public.user ("email", "nickname", "password_hash") VALUES($1, $2, $3)`
